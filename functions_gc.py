...
def omegaMuNu(t,dim1,dim2,Reg,Amunu,Omega_0,omega_L,energy_eigenvalues,energy_eigenvalues1,E0_t=0):
    ...
    energy_diff = np.array(energy_eigenvalues1[ np.newaxis,:] - energy_eigenvalues[:,np.newaxis], dtype=complex)
    #allows to specify the time dependence of the electric field
    if E0_t.all()==0:
        E0_t = np.exp(-(t-10)**2/100)+np.exp(-(t-60)**2/100)+np.exp(-(t-110)**2/100)+np.exp(-(t-160)**2/100)
#np.exp(-t**2/200)#otherwise just uses a Gaussian pulse
        #E0_t = np.sum([np.exp])
    exp_f = np.exp(1j * (energy_diff[:,:,np.newaxis] - omega_L) * t)*E0_t[np.newaxis,np.newaxis,:]
   
    Om=Omega_0*Amunu[:,:,np.newaxis]*exp_f
    # Om=Deg*E0(t)
   
    return Om


def system_equations_opt_vect(t, y, dim1, dim2, Reg, De1, Re1, we1, De2, Re2, we2,Gamma,Amunu,Omega_0,omega_L,energy_eigenvalues,energy_eigenvalues1,E0_t):
    """
    ...
    Omega=np.zeros_like(rho,dtype=complex)
...
def solve_dyn_vect(rho_initial, t, Reg, De1, Re1, we1, De2, Re2, we2, Esp,Omega_0,omega_L, method_code,wc=0,E0_t=0):
    ...
    solution = solve_ivp(system_equations_opt_vect, [0, tau], rho_initial_vector,
                        args=(dim1, dim2, Reg, De1, Re1, we1, De2, Re2, we2, Gamma,Amunu,Omega_0,omega_L,energy_eigenvalues,energy_eigenvalues1,E0_t),
                        method=method, t_eval=t, rtol=1e-10, atol=1e-12)
 
    return solution

def cycle_evolution(rho_initial, t, t_decay, Reg, De1, Re1, we1, De2, Re2, we2, Esp,Omega_0,omega_L, method_code,wc=0,E0_t=0):
    """This describes the evolution of the system over a cycle of driving pulse (given by time vector t) followed by spontaneous decay for a time t_decay"""
    lambd1 = lambd(De1,we1)
    lambd2 = lambd(De2,we2)
    energy_eigenvalues = omega_nu(lambd1) * De1
    energy_eigenvalues1 = omega_nu(lambd2) * De2+Esp
    ...
    Gamma = gammaMatrix(dim1, dim2, Reg, De1, Re1, we1, De2, Re2, we2, Esp,wc=wc)
    N = dim1 + dim2
    rho_inter = vector_to_rho(solve_dyn_vect(rho_initial, t, Reg, De1, Re1, we1, De2, Re2, we2, Esp, Omega_0, omega_L, method_code),N)#driving pulse evolution
    rho_final = Decay(rho_inter,dim1,Gamma,t_decay)#decay evolution
    
    return rho_final

def complete_evolution(rho_initial, t, t_decay, Ncycles, Reg, De1, Re1, we1, De2, Re2, we2, Esp,Omega_0,omega_L, method_code,wc=0,E0_t=0):
    """Repeats cycle evolution for Ncycles times"""
    Nt = 0
    rho_t = rho_initial
    while Nt<Ncycles:
        rho_t = cycle_evolution(rho_initial, t, t_decay, Reg, De1, Re1, we1, De2, Re2, we2, Esp,Omega_0,omega_L, method_code,wc=0,E0_t=0)
        N = N+1
    return rho_t
...
def DecayPopsMatrix(N,dim1,Gamma,t): 
    """gives the matrix that determines the decay of the populations (which are all coupled to each other)"""
    G_pops=np.zeros((N,N)) #N = dim1 + dim2 is the total number of levels
    G_pops[:dim1,dim1:] = Gamma #this is the block that gives the decay of the s populations as function of the p populations
    G_pops[dim1:,dim1:] = -np.diag(Gamma.sum(axis=0))  #this is the block that gives the decay of the p populations as function of the p populations
    return sla.expm(G_pops*t)


def Decay(rho,dim1,Gamma,t):
    """gives the density matrix evolution after a time t subject only to spontaneous decay with coefficients matrix Gamma_munu"""
    N=np.shape(rho)[0]
    M_pops = DecayPopsMatrix(N, dim1, Gamma, t)
    newpops = np.dot(M_pops,np.diag(rho)) #get vector with new populations
    
    newrho = rho.copy() #create new density matrix
    #all the coherences decay independently of each other, so we need to perform an element-wise multiplication (instead of matrix multiplication)
    v=Gamma.sum(axis=0) 
    M_pp = np.exp(-0.5*(np.outer(v,np.ones(N-dim1))+np.outer(np.ones(N-dim1),v))*t) #the rho_nup,nu'p coherences decay with this rate
    newrho[dim1:,dim1:] = M_pp*newrho[dim1:,dim1:] #note that this also change the diagonal (aka the populations) in the p levels. However the semi-last line takes care of that
    
    M_ps = np.exp(-0.5*np.outer(v,np.ones(dim1))*t) #the rho_nup,mus coherences decay with this rate
    newrho[dim1:,:dim1] = M_ps*newrho[dim1:,:dim1]
    newrho[:dim1,dim1:] = np.conj(np.transpose(newrho[dim1:,:dim1]))
    
    np.fill_diagonal(newrho, newpops)#fill the diagonal with the updated populations
    return newrho
